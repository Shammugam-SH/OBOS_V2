...
?>
<br>
<table class="table table-bordered" id="manageAnalysisCustomerQuantityGetDataTable" width="100%" cellspacing="0">
    <thead>
        <tr>
            <th>NO</th>
            <th>CUSTOMER ID</th>
            <th>CUSTOMER NAME</th>
            <th>TOTAL PURCHASED QUANTITY (UNIT)</th>
        </tr>
    </thead>
    <tbody>
        <?php
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>";
                echo "<input type='hidden' id='manageAnalysisCustomerQuantityGetDataTableHiddenInput' name='customerManagementPageTableHiddenInput' value='" . $longUserData . "' readonly>";
                echo "<td>" . $num . "</td>";
                echo "<td>" . $row["userEmail"] . "</td>";
                echo "<td>" . $row["userFName"] . " " . $row["userLName"] . "</td>";
                echo "<td>" . (int) $row["proQty"] . "</td>";
                echo "</tr>";
...
<script>
    $(document).ready(function () {
        ...
        $('#manageAnalysisCustomerQuantityGetDataTable').DataTable({
            "language": {
